...
type ArtifactType = 'line' | 'text' | 'interference';
...
export const GlitchOverlay: React.FC<GlitchOverlayProps> = ({ isActive, delay = 1000 }) => {
  ...
  useEffect(() => {
    if (!showEffect || !canvasRef.current) {
      return () => undefined;
    }
    ...
    const resizeCanvas = () => {
      ...
      noiseTextureRef.current = createNoiseTexture(ctx, canvas.width, canvas.height);
    };
